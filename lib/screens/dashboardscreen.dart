...
class DashboardScreen extends StatefulWidget {
  ...
  const DashboardScreen({Key? key, required this.userId, required this.token}) : super(key: key);
  ...
  Future<void> fetchDashboardData() async {
    try {
      ...
      if (response.statusCode == 200) {
        ...
        // Extract order data
        final orders = responseData.map((order) {
          final orderDate = DateTime.parse(order["createdAt"]).toLocal().toString().split(' ')[0];
          final customerName = order["customerName"] ?? "Unknown";
          final quantity = order["products"].fold<int>(
            0,
    ...
    return Scaffold(
      bottomNavigationBar: BottomNavigationBar(
        ...
        items: const [
          ...
          BottomNavigationBarItem(icon: Icon(Icons.inventory), label: 'Manage Products'),
        ],
      ),
      body: SafeArea(
        child: isLoading
